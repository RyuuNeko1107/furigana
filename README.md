...
| **インストール後のバイナリ名** | **`furigana`** | `furigana lookup ...` で実行 |
| **GitHub repo (本体)** | **`RyuuNeko1107/ja-furigana`** | このリポジトリ |
| **GitHub repo (辞書)** | **`RyuuNeko1107/ja-furigana-dict`** | 辞書 PR はここに |

`furigana` という crate 名は別 OSS に先に取られていたため、crate 名としては `ja-` prefix 付きで公開しています。
ただし Rust から使うときの `use ...` はライブラリ側で `[lib] name = "furigana"` を指定しているため
`use furigana::Furigana;` (アンダースコアではなくそのまま `furigana`) が正しい形です。
バイナリ名と CLI コマンド名も `furigana` (打ちやすさ優先で旧来の名前を残置)。
...
- **ライブラリ** (`ja-furigana` crate): `cargo add ja-furigana` で組み込める。`use furigana::Furigana;` で使う ([名前の対応](#名前の対応-混乱しやすい点) を参照)。DB 不要、async 不要、Pure Rust。
...
`mode` は `tts` (default) | `hiragana` | `ruby` | `kanji` | `romaji` (ヘボン式) | `romaji-kunrei` (訓令式) の 6 つ。
...
`<data_dir>/config.toml` を読みます。default の `<data_dir>` は **実行ファイルと同じディレクトリ** (portable 配置)。`furigana.exe` を解凍したフォルダの中の `config.toml` がそのまま使われます。`--data-dir <path>` または `FURIGANA_DATA_DIR` で別の場所に変えられます。`current_exe()` の解決に失敗した稀なケースでのみ XDG (`~/.local/share/furigana/config.toml` / `%LOCALAPPDATA%\furigana\furigana\config.toml`) に fallback します。
...
- ✅ 速度最適化 (Lindera analyzer の lazy init: `Furigana::minimal()` 単体で 5.97 ms → 27.3 µs、`--version` / `--help` 等の Lindera 不要経路で ~80 ms → ~10 ms)
